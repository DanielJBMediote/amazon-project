...
class Webdriver {
  constructor(props) {
    this.driver = new Selenium.Builder().forBrowser(props.browser).build();
  ...
  async boot() {
    try {
      ...
      await this.buttonClick(Selenium.By.xpath(elXPaths.LINKS.loginButton));
  ...
  async loopAmazonCountry(numberOfCountry) {
    ...
    do {
      await this.waitPageLoad();
      await this.driver.wait(Selenium.until.elementsLocated(
        Selenium.By.xpath(
          "/html/body/div[1]/div[1]/div/div[6]/div[2]/div[4]/div/select/optgroup/option[" + indexOption + "]"
        )));
      ...
      if (countryName == optionText) {
        ...
        await this.buttonClick(
          Selenium.By.xpath("/html/body/div[1]/div[1]/div/div[6]/div[2]/div[4]/div/select/optgroup/option[" + indexOption + "]"
          ));
  ...
  async loopPages() {
    ...
    let totalTabs = await this.driver
      .wait(Selenium.until.elementLocated(
        Selenium.By.xpath(elXPaths.ELEMENTS.totalTabs)
      ), 7000)
      .then((el) => { return el.getAttribute("innerHTML") });

    let totalProducts = await this.driver
      .wait(Selenium.until.elementLocated(
        Selenium.By.xpath(elXPaths.ELEMENTS.totalProducts)
      ), 7000)
      .then((el) => { return el.getAttribute("innerHTML") });
    ...
    consoleQuest.question("Chosse the page of 1 to " + totalTabs + ". [Press 0 to Exit]", async page => {
      if (page != 0) {
        ...
        if (page != 1) {
          await (await this.driver.findElement(Selenium.By.xpath(elXPaths.ELEMENTS.tabInput))).clear();
          await this.driver.findElement(Selenium.By.xpath(elXPaths.ELEMENTS.tabInput)).sendKeys(currentTab);
          await (await this.driver.findElement(Selenium.By.xpath(elXPaths.LINKS.buttonTabGO))).click();
          await this.waitPageLoad();
        }
          ...
          if (priceElBussiness == "") {
            (await (await this.driver).findElement(
              Selenium.By.xpath(
                ...
                `div[5]/div/table/tbody/tr[${index + 1
